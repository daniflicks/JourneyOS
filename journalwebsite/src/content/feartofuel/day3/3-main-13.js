...
// Component for guided reflection on Day 3 - Lessons from Most Painful Memory
export default function Day3Main13({
  answers,
  onChange,
  onContinue,
  aiResponse,
  ...
  const [mostPainfulSummary, setMostPainfulSummary] = useState('');

  // Local buffer for user input, separate from committed answer
  const [localReflection, setLocalReflection] = useState(
    answers?.day3LessonsFromMostPainfulMemory || ''
  );

  // Get preloaded most painful failure summary from Day 3
  useEffect(() => {
    const mostPainfulFailureStory = answers?.day3MostPainfulFailureReflection || ''; 
    if (mostPainfulFailureStory) {
      const cacheKey = `claude-3-${SECTION_TYPES.MAIN_EXERCISE}-11-${encodeURIComponent(mostPainfulFailureStory)}`;
      const cached = localStorage.getItem(cacheKey);      
      if (cached) {
        setMostPainfulSummary(cached);
      }
    }
  ...
  useEffect(() => {
    if (
      answers?.day3LessonsFromMostPainfulMemory !== undefined &&
      !showAIResponse
    ) {
      setLocalReflection(answers.day3LessonsFromMostPainfulMemory);
    }
  }, [answers?.day3LessonsFromMostPainfulMemory, showAIResponse]);

  // Track whether user has unsaved edits
  const committed = answers?.day3LessonsFromMostPainfulMemory || '';
  ...
  const handleGetInsight = () => {
    ...
    onChange('day3LessonsFromMostPainfulMemory', localReflection);
    setShowAIResponse(true);
  };
